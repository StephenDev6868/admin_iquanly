					...
					<tbody>
					  ...
					  @foreach($users as $user)
						<tr id="row_{{ $user->id }}">
							...
							<td class="text-center">
							  <form action="{{ action('UserStaffController@destroy', $user['id']) }}" method="post">
								<a href="{{ action('UserStaffController@edit', $user['id']) }}" data-title="{{ _lang('Update User') }}" class="btn btn-outline-warning btn-xs ajax-modal">{{ _lang('Edit') }}</a>
								<a href="{{ action('UserStaffController@show', $user['id']) }}" data-title="{{ _lang('View User') }}" class="btn btn-outline-primary btn-xs ajax-modal">{{ _lang('View') }}</a>
								{{ csrf_field() }}
